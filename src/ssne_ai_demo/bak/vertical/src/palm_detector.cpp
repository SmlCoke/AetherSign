...
void PALMDETECTOR::Initialize(const std::string& model_path,
    ...
    std::cout << "[PALM] Manual preprocessing: camera=" << image_shape[0] << "x" << image_shape[1]
              << " -> bilinear resized=" << input_shape[0] << "x" << input_shape[1] << std::endl;
...
void PALMDETECTOR::Predict(ssne_tensor_t* img,
    ...
    if (!PreprocessResize(*img, timing)) {
        if (timing != nullptr) {
...
bool PALMDETECTOR::PreprocessResize(ssne_tensor_t camera_tensor, PalmPredictTiming* timing) {
    ...
    ResizeBilinear(camera_data,
                   ...
                   input_shape[1]);
    if (timing != nullptr) {
        timing->preprocess_resize_ms = PredictDurationMs(stage_begin, PredictClock::now());
    }
...
PALMDETECTOR::Anchor PALMDETECTOR::GetAnchor(int feature_size,
    ...
    if (feature_size == kPalmFeature14) {
        anchor.w = anchor_index == 0 ? 0.084651f : 0.104590f;
        anchor.h = anchor_index == 0 ? 0.032030f : 0.079621f;
    } else {
        anchor.w = anchor_index == 0 ? 0.137044f : 0.157819f;
        anchor.h = anchor_index == 0 ? 0.049292f : 0.088213f;
    }
    return anchor;
}
...
PalmKeypoint PALMDETECTOR::MapPoint(float model_x, float model_y) const {
    ...
    float original_x = point.model_x * static_cast<float>(image_shape[0] - 1);
    float original_y = point.model_y * static_cast<float>(image_shape[1] - 1);
