...
const float kPalmHighLevelAnchorShapes[kPalmNumAnchorsPerCell][2] = {
    {0.1188828125f, 0.2170138889f},
    {0.1298750000f, 0.3417500000f},
};

const float kPalmLowLevelAnchorShapes[kPalmNumAnchorsPerCell][2] = {
    {0.1716406250f, 0.2737222222f},
    {0.1932968750f, 0.4076527778f},
};
...
void PALMDETECTOR::Initialize(const std::string& model_path,
    ...
    if (use_ai_preprocess) {
        ...
        if (set_norm_ret != 0) {
            std::cerr << "[PALM] Warning: SetNormalize failed, ret=" << set_norm_ret
                      << ". Manual gray input will still be generated." << std::endl;
        } else {
            std::cout << "[PALM] AI preprocess normalization was configured from the model." << std::endl;
        }
    } else {
        std::cout << "[PALM] AI preprocess is bypassed. The resized gray buffer will be copied"
    ...
    std::cout << "[PALM] Detector anchors: head14x24="
              << (kPalmFeatureHighHeight * kPalmFeatureHighWidth * kPalmNumAnchorsPerCell)
              << ", head7x12="
              << (kPalmFeatureLowHeight * kPalmFeatureLowWidth * kPalmNumAnchorsPerCell)
...
size_t PALMDETECTOR::ExpectedRegElements(int feature_width, int feature_height) {
    return static_cast<size_t>(feature_width) *
           static_cast<size_t>(feature_height) *
           static_cast<size_t>(kPalmRegChannels);
}

size_t PALMDETECTOR::ExpectedClsElements(int feature_width, int feature_height) {
    return static_cast<size_t>(feature_width) *
           static_cast<size_t>(feature_height) *
...
PALMDETECTOR::OutputMapping PALMDETECTOR::MapOutputs(const TensorDebugInfo output_info[kPalmOutputCount]) const {
    ...
    const size_t expected_values[kPalmOutputCount] = {
        ExpectedRegElements(kPalmFeatureHighWidth, kPalmFeatureHighHeight),
        ExpectedClsElements(kPalmFeatureHighWidth, kPalmFeatureHighHeight),
        ExpectedRegElements(kPalmFeatureLowWidth, kPalmFeatureLowHeight),
        ExpectedClsElements(kPalmFeatureLowWidth, kPalmFeatureLowHeight),
    };
...
PALMDETECTOR::Anchor PALMDETECTOR::GetAnchor(bool high_level,
                                             int feature_width,
                                             int feature_height,
                                             int cell_x,
                                             int cell_y,
    ...
    Anchor anchor;
    const float step_x = 1.0f / static_cast<float>(feature_width);
    const float step_y = 1.0f / static_cast<float>(feature_height);
    anchor.cx = static_cast<float>(cell_x) * step_x + step_x * 0.5f;
    anchor.cy = static_cast<float>(cell_y) * step_y + step_y * 0.5f;

    if (high_level) {
        anchor.w = kPalmHighLevelAnchorShapes[anchor_index][0];
        anchor.h = kPalmHighLevelAnchorShapes[anchor_index][1];
    } else {
        anchor.w = kPalmLowLevelAnchorShapes[anchor_index][0];
        anchor.h = kPalmLowLevelAnchorShapes[anchor_index][1];
    }
    return anchor;
}
...
size_t PALMDETECTOR::GetOutputIndex(int feature_width,
                                    int feature_height,
                                    int channel_count,
                                    int channel,
                                    int cell_x,
    ...
    const size_t spatial = static_cast<size_t>(feature_width) * static_cast<size_t>(feature_height);
    const size_t cell_index = static_cast<size_t>(cell_y) * static_cast<size_t>(feature_width) +
...
void PALMDETECTOR::DecodeHead(ssne_tensor_t reg_tensor,
    ...
    const size_t expected_reg = ExpectedRegElements(feature_width, feature_height);
    const size_t expected_cls = ExpectedClsElements(feature_width, feature_height);
    if (reg_info.inferred_elements < expected_reg || cls_info.inferred_elements < expected_cls) {
        std::cerr << "[PALM] Output element count too small for head "
    ...
    for (int y = 0; y < feature_height; y++) {
        for (int x = 0; x < feature_width; x++) {
            for (int anchor_idx = 0; anchor_idx < kPalmNumAnchorsPerCell; anchor_idx++) {
                ...
                const float box_w = anchor.w * std::exp(std::max(-kPalmRegOffsetClip, std::min(kPalmRegOffsetClip, dw)));
                const float box_h = anchor.h * std::exp(std::max(-kPalmRegOffsetClip, std::min(kPalmRegOffsetClip, dh)));
                ...
                detection.head_feature_size = high_level ? kPalmFeatureHighHeight : kPalmFeatureLowHeight;
...
void PALMDETECTOR::DecodeOutputs(const OutputMapping& mapping,
    ...
    DecodeHead(outputs[mapping.reg14],
               ...
               true,
               kPalmFeatureHighWidth,
               kPalmFeatureHighHeight,
               layout,
               &candidates);
    DecodeHead(outputs[mapping.reg7],
               ...
               false,
               kPalmFeatureLowWidth,
               kPalmFeatureLowHeight,
               layout,
               &candidates);
