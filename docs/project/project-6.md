...
| 成员 | 分工 | 
|---|---|
| Peak | 尝试在板上部署三个 Mediapipe 模型之一的 palm 检测模型，成功；写文档 |
| Dragon | 继续优化 MediaPipe 的精度，写文档 |
| Soar | 尝试在板上部署 Mediapipe 模型，写文档 |

**进展**
- Peak: 已经画好了所有模型（MediaPipe 的 Palm Detector, Hand Landmarker, Pose Landmarker）。0505日中午成功部署好了 Palm Detector 模型，板上运行成功，能够实时检测手掌位置并且输出 bounding box，正在尝试获取性能数据。0505日完成功添加好日志，获取性能数据。0506日凌晨成功部署好了 Hand Landmarker 模型，板上运行成功，能够实时检测手部位置并且输出21个关键点坐标，但是存在精度不高的问题，手指关键点连线与真实双手位置存在一定偏差，正在尝试解决问题
- Dragon: 继续调整 MediaPipe PC 端的精度，写文档
- Soar: 尝试在板上部署 Mediapipe 模型，写文档

所有模型均已完成算子适配以及工具链转化，正在进行部署测试。
